...
function shell_env() {
  ...
  case "$shell_name" in
    bash)
      ...
      if ! grep -q "source <(ksim completion bash)" "$home_user"/.bashrc; then
          echo "source <(ksim completion bash)" >> "$home_user"/.bashrc
      fi
      ;;
    zsh)
      ...
      if ! grep -q "source <(ksim completion zsh)" "$home_user"/.zshrc; then
          echo "source <(ksim completion zsh)" >> "$home_user"/.zshrc
      fi
      ;;
    fish)
      ...
      if ! grep -q "ksim completion fish | source" "$home_user"/.config/fish/config.fish; then
          echo "ksim completion fish | source" >> "$home_user"/.config/fish/config.fish
      fi
      ;;
    csh)
